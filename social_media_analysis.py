...
def social_media_analysis(api_key, object_ids, timestamp_start, timestamp_end):
    ...
    object_id_list = [{'id': item} for item in object_ids]
    id_list = []

    for item in keyword_data['result']:
        for data in object_id_list:
            if item['id'] == data['id']:
                new_data = {
                    'id': item['id'],
    ...
    if top_post_made_data_list:
        ...
        if highest_engagement_rate_data:
            platform_post_highest_engagement_rate_list = request_top_post_made(api_key=api_key,
                                                                            timestamp_start=timestamp_start,
                                                                            timestamp_end=timestamp_end,
